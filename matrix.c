...
else {if (bb == 0.0) return 0; else return (bb * sqrt (1.0 + SQR(aa/bb))); }
}
////////////////////////////////////////////

indices getind (int index, int nmax)
{
indices a;
a.i = index%4;
index /= 4;
...
prod = (xexp+1) * (yexp+1) * (zexp+1);
if (prod %2 == 0 && a.geom!=4) 
{    
    return 0;  // if any of the exponents are odd, integral is zero
}
else 
{
    switch (a.geom)
    ...
    case 4: //Potato geometry
        ...
        //Sum on quadrants
        integral=0.;
        if((xexp%2)+(yexp%2)+(zexp%2)>1) {mm=1.;}
        else {mm=PI/2.;}
...
odidr=a.OD/a.ID;
   ...
   //   integral=((pow(a.OD,xexp+yexp+2)- pow(a.ID,xexp+yexp+2)) * pow(a.dz, zexp+1))*PI*(p*r*4./(zexp+1)/t);
     ...
     // if((integral-integral2)!=0) printf ("Integrals are different.\n");
...
/* Returns the transpose of a square matrix of dimension 'dim' */
...
void
tri_eig (double *d, /* input diagonal elements */
...
/* Make the 'C' matrix for Cholescky decomposition*/
...
[38] icalc  0 = fit, !=0 = number of lines to calculate
...
double *wt = vector(1000);
 ...
 while(cont)	
 {
  ...
  {
      sscanf (buffer, "%i %i %i %i %i %lg %lg %i",&iparm[45],&iparm[36],&iparm[42],&iparm[38],&iparm[46],&param[40],&param[41],&iparm[39]);
      cont=false;
  }
 }
...
/* Generates lookup tables. Either with or without 8 mirror planes */
...
double *dparam=vector(55);
long tempderiv[1];
long tempfsym[1];
// In all case ideriv should be <=15 and never be 12
for(i=1;i<55;i++) {dparam[i]=param[i];}
...
nfree=ndata-nterms; // real chisqr
nfree=ndata; // similar to Migliori et al

iparm[41]=nterms;

if(nfree<=0) {return 0;} //Nothing to fit.
...
*chisqr0=fchisq(fex,fth,w,fsym,iparm[44],nfree); // iparm[24] is the number of data points. 

// //////////////////////////////////////////////////////
printf("---- Initial Chisqr: %lg\n",*chisqr0);
//print_vector((double *)fex[0],iparm[24]);print_vector((double *)fth[0],iparm[24]);
...
while(next)
    {
    next=0;
    for(j=1;j<=nterms;j++) {for(k=1;k<=nterms;k++) array[j][k]=alpha[j][k]/sqrt(alpha[j][j]*alpha[k][k]);array[j][j]=1.+*flambda;}

    // invert array with Shipley algorithm
    for(k=1;k<=nterms;k++)                
        {
    ...
    for(i=1;i<54;i++) {b[i]=param[i];} // Use old parameters as starting value.
    for(j=1;j<=nterms;j++) for(k=1;k<=nterms;k++) b[dvar[j]]=b[dvar[j]]+beta[k]*array[j][k]/sqrt(alpha[j][j]*alpha[k][k]);
    ...
    solve_freq(lookup,b,iparm,dfact,c,fth,fsym);
    ...
    *chisqr1=fchisq(fex,fth,w,fsym,iparm[44],nfree);
    ...
    if((*chisqr0-*chisqr1)<=-0.00001) {*flambda=*flambda*10.;next=1;printf("---- Step Chisqr:    %lg\n",*chisqr1);}
    }
	   
//printf("\n");
printf("---- Final Chisqr:   %lg\n",*chisqr1);

for(i=1;i<55;i++) {param[i]=b[i]; if(b[i]<0) printf("==== Warning: parameter %i found negative.\n",i);} // New solution found. c

//print_vector(param,15);

for(j=1;j<=nterms;j++) sigmap[j]= sqrt(array[j][j]/alpha[j][j]);
//print_vector (sigmap,nterms); 
*flambda=*flambda/10.;
free_matrix(array);
free_matrix(alpha);
free_vector(beta);
      ...
      {
      print_vector((double *)fth[0],iparm[38]); // Used only in the rare case that all parameters are fixed.
      *chisqr1=1.;
      }
        free_ivector(dvar);
        free_table(deriv);
...
[30]-[32] dimensions, in cm
[50]-[52] dimensions, in cm (xm,ym,zm for potato)
...
[37] numdim 0 = fixed dimensions, >0 = free dimensions
[38] icalc  0 = fit, !=0 = number of lines to calculate
[39] iprint 0 = no visual output, 1 = output (not used at the moment)
[40] iteration counter in curfit
[41] nterms = number of fit parameters
[42] nmax, order of the polynomials
[43] dim, computer generated, dimension of the full matrix
[44] n, number of experimental peaks that are being read.
[45] shape. 5 = sphere; 1 = RPR; 0 = cylinder
[46] has mirror planes   1 = yes; 0 = no
*/
...
fprintf(fp,"Error on free parameters:\n");
...
for(i=1;i<22;i++) 
    {
     if(iparm[i]==0) fprintf(fp," %6.4f %% ",rms2*sigmap[dvar[i]]/param[i]*10.); 
    }  
...
if(iparm[30]==0)
    ...
    ed1=rms2*sigmap[dvar[30]]/param[30];
    ed2=rms2*sigmap[dvar[31]]/param[31];
    ed3=(vo/d1*ed1+vo/d2*ed2)/(vo/d3);
    fprintf(fp," %6.4f   %6.4f   %6.4f ",ed1,ed2,ed3);
    }   
for(i=33;i<36;i++) 
    {
//     if(iparm[i]==0) fprintf(fp," %lg %% ",rms2*sigmap[i]/param[i]*100.); //Need to assess if factor pi/60 is needed here.
             if(iparm[i]==0) fprintf(fp," %6.4f    ",rms2*sigmap[dvar[i]]); //Need to assess if factor pi/60 is needed here.
    }  
